...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const cartData = await getCarts({ token });
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      router.replace(`/user/checkout/transaction/${latestTransaction.id}`);
    } catch (err: any) {
      alert(err.message);
    } finally {
      setSubmitting(false);
          ...
          {!loading && !error && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="md:col-span-2 space-y-4">
                {cartItems.map((item) => (
                  ...
                  >
                    ...
                    />
                    <div>
                      <p className="font-semibold text-[#F1F0E4]">
                        {item.food.name}
                      </p>
              ...
              <div className="border-[#7D8D86]  bg-[#7D8D86] rounded-lg p-4 h-fit">
                <h2 className="font-semibold mb-4 text-[#F1F0E4]">
                  Payment Method
                </h2>
                ...
                <div className="space-y-2 mb-4">
                  {paymentMethods.map((pm) => (
                    <label
                      key={pm.id}
                      className="flex items-center text-[#F1F0E4] gap-2"
                    >
                      <input
                        type="radio"
